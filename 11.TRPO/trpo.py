...
class TRPO:
    ...
    def hessian_matrix_vector_product(self, states, old_action_dist, vector):
        # 计算hessian矩阵和一个向量的乘积
    ...
    def conjugate_gradient(self, grad, states, old_action_dists):  # 共轭梯度法求解方程
        ...
        for i in range(10):  # 共轭梯度主循环
            Hp = self.hessian_matrix_vector_product(states, old_action_dists, p)  # 计算hessian矩阵和向量乘积
    ...
    def line_search(self, states, actions, advantage, old_log_probs, old_action_dists, max_vec):  # 线性搜索
        ...
        for i in range(15):  # 线性搜索主循环
            ...
            kl_div = torch.mean(torch.distributions.kl.kl_divergence(old_action_dists, new_action_dist))
    ...
    def policy_learn(self, states, actions, old_action_dists, old_log_probs, advantage):  # 更新策略函数
        ...
        new_para = self.line_search(states, actions, advantage, old_log_probs,
                                    old_action_dists, descent_direction * max_coef)  # 线性搜索
        torch.nn.utils.convert_parameters.vector_to_parameters(new_para, self.actor.parameters())  # 更新参数
    ...
    def update(self, transition_dict):
        ...
        td_delta = td_target - self.critic(states)  # 优势函数
...
return_list = rl_utils.train_on_policy_agent(env, agent, num_episodes, render=True)
