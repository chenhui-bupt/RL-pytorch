...
def bc_agent_test(agent, env, n_episode):
    return_list = []
    for episode in range(n_episode):
        episode_return = 0
        state = env.reset()
...
with tqdm(total=n_iterations, desc="进度条") as pbar:
    for i in range(n_iterations):
        ...
        current_return = bc_agent_test(bc_agent, env, 5)
...
class GAIL:  # generative adversarial imitation learning
    ...
    def learn(self, expert_s, expert_a, agent_s, agent_a, next_s, dones):
        ...
        transition_dict = {'states': agent_s, 'actions': agent_a, 'rewards': rewards,
                           'next_states': next_s, 'dones': dones}
