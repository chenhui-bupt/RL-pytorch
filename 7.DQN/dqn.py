...
class ReplayBuffer:
    ...
    def add(self, state, action, reward, next_state, done):  # 将数据加到buffer
        self.buffer.append((state, action, reward, next_state, done))

    def sample(self, batch_size):  # 从buffer中随机抽取batch_size个数据
        transitions = random.sample(self.buffer, batch_size)
        states, actions, rewards, next_states, dones = zip(*transitions)
        return np.array(states), actions, rewards, np.array(next_states), dones

    def size(self):  # 目前buffer中数据量
...
class DQN:
    ...
    def __init__(self, state_dim, hidden_dim, action_dim, learning_rate, gamma, epsilon, target_update, device):
        ...
        self.target_update = target_update  # 更新目标网络频率
        self.count = 0  # 更新次数
        self.device = device

    def take_action(self, state):  # e-贪婪策略采取动作
    ...
    def update(self, transition_dict):
        ...
        self.optimizer.zero_grad()  # pytorch中默认梯度会累加，需要手动清零
