...
class CliffWalkingEnv:
    ...
    def __init__(self, ncol=12, nrow=4):
        self.ncol = ncol  # 定义网格的列
        self.nrow = nrow  # 定义网格的行
    ...
    def createP(self):
        ...
        for i in range(self.nrow):
            for j in range(self.ncol):
                for a in range(4):
                    # 位置在悬崖或者目标状态，因为无法继续交互，任何动作奖励都是0
...
class PolicyIteration:
    ...
    def __init__(self, env, theta, gamma):
        ...
        self.pi = [[0.25, 0.25, 0.25, 0.25] for i in range(self.env.ncol * self.env.nrow)]  # 初始化为随机均匀策略
    ...
    def policy_evaluation(self):  # 策略评估
        ...
        while 1:
            ...
            for s in range(self.env.ncol * self.env.nrow):
                ...
                new_v[s] = sum(qsa_list)  # 状态价值函数和动作价值函数之间的关系（求边缘之和）
