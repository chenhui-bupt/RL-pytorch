...
class PolicyNet(torch.nn.Module):
    ...
    def forward(self, x):
        x = F.relu(self.fc1(x))
        mu = self.fc_mu(x)
        std = F.softplus(self.fc_std(x))  # 保证标准差不小于0
        dist = Normal(mu, std)
    ...
    def truncated_normal_init(t, mean=0.0, std=0.01):
        ...
        while True:
            ...
            if not torch.sum(cond):
                break
            t = torch.where(cond, torch.nn.init.normal_(torch.ones(t.shape, device=device),
                                                  mean=mean, std=std), t)
        return t
...
class MBPO:
    def __init__(self, env, agent, fake_env, env_pool, model_pool, rollout_length,
                 rollout_batch_size, real_ratio, num_episode):
        self.env = env
        self.agent = agent
    ...
    def update_agent(self, policy_train_batch_size=64):
        ...
        for epoch in range(10):
            ...
            transition_dict = {'states': obs, 'actions': action, 'rewards': reward,
                               'next_states': next_obs, 'dones': done}
