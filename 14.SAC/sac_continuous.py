...
minimal_size = 1000
batch_size = 64
target_entropy = -env.action_space.shape[0]  # 动作
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

replay_buffer = rl_utils.ReplayBuffer(buffer_size)
agent = SACContinuous(state_dim, hidden_dim, action_dim, action_bound, actor_lr, critic_lr, alpha_lr,
                      target_entropy, tau, gamma, device)
return_list = rl_utils.train_off_policy_agent(env, agent, num_episodes, replay_buffer, minimal_size, batch_size, render=True)
