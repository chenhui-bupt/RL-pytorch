...
class SAC:
    ...
    def __init__(self, state_dim, hidden_dim, action_dim, actor_lr, critic_lr, alpha_lr,
        ...
        self.target_entropy = target_entropy  # 目标熵约束
        self.gamma = gamma
        self.tau = tau
        self.device = device
    ...
    def calc_target(self, rewards, next_states, dones):
        ...
        entropy = -torch.sum(next_probs * next_log_probs, dim=1, keepdim=True)  # 熵：plog(p)
    ...
    def update(self, transition_dict):
        ...
        actions = torch.tensor(transition_dict['actions']).view(-1, 1).to(self.device)  # 动作是离散的，不再是float类型
...
print('state_dim: %d, action_dim: %d' % (state_dim, action_dim))
