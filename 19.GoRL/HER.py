...
class WorldEnv:
    ...
    def step(self, action):
        ...
        dis = np.sqrt(np.sum(np.square(self.state - self.goal)))  # 欧氏距离
        reward = -1.0 if dis > self.distance_threshold else 0.0
...
class ReplayBuffer_Trajectory:
    ...
    def sample(self, batch_size, use_her, dis_threshold=0.15, her_ratio=0.8):
        ...
        for _ in range(batch_size):
            ...
            next_state = traj.states[step_state+1]  #todo: 是否越界
...
for i in range(10):
    with tqdm(total=int(num_episodes/10), desc='Iteration %d' % i) as pbar:
        for i_episode in range(int(num_episodes/10)):
            ...
            if (i_episode+1) % 10 == 0:
                pbar.set_postfix({'episode': '%d' % (num_episodes/10 * i + i_episode + 1), 'return': np.mean(return_list[-10:])})
            pbar.update(1)
